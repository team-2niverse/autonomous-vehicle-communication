...
static err_t sota_accept_callback(void *arg, struct tcp_pcb *newpcb, err_t err)
{
    ...
    my_printf("New client connected.\n");
...
static void sota_error_callback(void *arg, err_t err)
{
    LWIP_UNUSED_ARG(err);
    SotaSession *ss = (SotaSession*)arg;
    my_printf("TCP Error occurred. Closing session.\n");
    if (ss != NULL)
    {
...
static void handle_hex_line(struct tcp_pcb *tpcb, SotaSession *ss, const char *line)
{
    ...
    if (ss->state == SS_IDLE && strncmp(line, "START", 5) == 0)
    {
        ...
        my_printf("Received START command. State -> RECEIVING_HEX\n");
    ...
    if (ss->state == SS_RECEIVING_HEX)
    {
        ...
        switch (record_type)
        {
            ...
            {
                my_printf("Received EOF Record. Finalizing session.\n");
            ...
            case 0x04: // Extended Linear Address Record
            {
                ...
                my_printf("Received Address Record (0x04). State -> WAIT_ERASE\n");
...
{
    struct tcp_pcb *pcb;
    my_printf("Initializing Sota Server...\n");

    pcb = tcp_new();
    if (pcb == NULL)
    {
        my_printf("Error creating PCB. Out of memory?\n");
        return;
    }
    ...
    my_printf("Server listening on port %d.\n", SOTA_TCP_PORT);
}
...
void sota_server_poll(void)
{
    ...
    if (ss->pcb != NULL && ss->state == SS_WAIT_ERASE)
    {
        ...
        if (!flashBusy)
        {
            ...
            my_printf("Erase done. State -> RECEIVING_HEX. Sending delayed ACK.\n");
            const char* msg = "ACK: Line OK.\r\n";
