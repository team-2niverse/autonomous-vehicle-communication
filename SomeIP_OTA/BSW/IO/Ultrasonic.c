...
float Ultrasonic_ReadRightSensor_noFilt(void)
{
    ...
    while (MODULE_P15.IN.B.P2 == 0) { // wait for ECHO_HIGH
        if (getTimeUs() - timeout > 50000) return 0.0f; // if echo pin does not triggered within 100ms, return 0
    }
    timer_start = getTimeUs();

    timeout = timer_start;
    while (MODULE_P15.IN.B.P2 == 1) { // wait for ECHO_LOW
        if (getTimeUs() - timeout > 50000) return 0.0f; // if echo pin does not go LOW within 100ms, return 0
    }
    timer_end = getTimeUs();
