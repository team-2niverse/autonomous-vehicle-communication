...
def handle_session(pc_socket):
    ...
    try:
        ...
        while True:
            ...
            # ECU와 연결이 되어있는 경우
            if is_ecu_connected:
                # ECU가 연결되어 있다면 메시지를 ECU로 전달하고
                # ECU로부터 응답을 받아 PC로 전달
                print("메시지를 ECU로 전달합니다...")
                ecu_socket.sendall(pc_data)

                # PC가 TesterPresent를 보낸 경우 응답을 기다리지 않음
                # ECU는 TesterPresent 요청에 대한 응답을 보내지 않음
                if len(pc_data) > 12 and pc_data[12] == 0x3E:
                    print("TesterPresent 메시지를 성공적으로 ECU에 전달했습니다.")
                    continue

                print("ECU로부터 응답 대기 중...")
                ecu_response = ecu_socket.recv(1024)
                if not ecu_response:                    
                    print("ECU와 연결이 끊겼습니다. NRC 모드로 동작합니다.")
            ...
            # ECU와 연결이 끊어져있는 경우
                ...
                # ECU 연결 재시도에 실패한 경우
                    ...
                    # is_ecu_connected는 False 유지
                    if len(pc_data) > 12 and pc_data[12] != 0x3E:
                        # PC가 TesterPresent를 보낸 경우 NRC 응답을 생성하지 않음
                        nrc_response = create_nrc_22_response(pc_data)
                        pc_socket.sendall(nrc_response)
