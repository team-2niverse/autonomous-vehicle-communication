...
static volatile unsigned int rpm0 = 0;
static volatile unsigned int rpm1 = 0;

static volatile unsigned char canSend = 0;
...
void Encoder_Init_Stm_100ms(void) {
    MODULE_STM1.CMCON.B.MSIZE1 = 31;
    MODULE_STM1.CMCON.B.MSTART1 = 0;
    MODULE_STM1.ICR.B.CMP1OS = 1;

    MODULE_SRC.STM.STM[1].SR[1].B.TOS = 0;
    MODULE_SRC.STM.STM[1].SR[1].B.SRPN = ISR_PRIORITY_STM1_CMP1;
    MODULE_SRC.STM.STM[1].SR[1].B.CLRR = 1;
    MODULE_SRC.STM.STM[1].SR[1].B.SRE = 1;

    MODULE_STM1.ISCR.B.CMP1IRR = 1U;
    MODULE_STM1.ICR.B.CMP1EN = 1U;

    MODULE_STM1.CMP[1].U = (unsigned int)((MODULE_STM1.TIM0.U | ((uint64)MODULE_STM1.CAP.U << 32)) + 100000*CPU_CLOCK_MHZ);
}
...
IFX_INTERRUPT(Encoder_Stm1_Cmp1_Isr_Handler, 0, ISR_PRIORITY_STM1_CMP1);
...
void Encoder_Stm1_Cmp1_Isr_Handler(void) {
    MODULE_STM1.CMP[1].U = (unsigned int)((MODULE_STM1.TIM0.U | ((uint64)MODULE_STM1.CAP.U << 32)) + 100000*CPU_CLOCK_MHZ);
