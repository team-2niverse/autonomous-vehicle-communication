...
// 공용 버퍼 생성(response message 저장)
#define UDS_RESPONSE_BUFFER_SIZE 256
static uint8 uds_response_msg[UDS_RESPONSE_BUFFER_SIZE];
...
//  LWIP_DEBUGF(ECHO_DEBUG, ("doip_recv\n"));
    ...
    if (p == NULL) {
        ...
        if (es->p == NULL) {
            ...
            volatile uint16 doip_payload_type = *(((uint8*)p->payload) + 2) << 8 | *(((uint8*)p->payload)+ 3);
            if(doip_payload_type == 0x8001) {
                uint16 uds_response_len = UDS_HandleRequest(p, uds_response_msg);
                if(uds_response_len > 0)
                {
                    uint8 doip_header[] =
                    {
                            0x02, 0xFD,
                    ...
                    uint32 doip_payload_len = 4 + uds_response_len;
                    doip_header[4] = (uint8_t)(doip_payload_len >> 24);
                    doip_header[5] = (uint8_t)(doip_payload_len >> 16);
                    doip_header[6] = (uint8_t)(doip_payload_len >> 8);
                    doip_header[7] = (uint8_t)(doip_payload_len);

                    uint16 final_len = sizeof(doip_header) + sizeof(doip_addr) + uds_response_len;
                    uint8 final_doip_message[final_len];

                    uint16 curr_pos = 0;
                    memcpy(&final_doip_message[curr_pos], doip_header, sizeof(doip_header));
                    curr_pos += sizeof(doip_header);
                    memcpy(&final_doip_message[curr_pos], doip_addr, sizeof(doip_addr));
                    curr_pos += sizeof(doip_addr);
                    memcpy(&final_doip_message[curr_pos], uds_response_msg, uds_response_len);

                    es->p->len = final_len;
                    memcpy(es->p->payload, final_doip_message, final_len);
